...
class Strategy:
    ...
    def predict_prob_embed(self, X, Y, eval=True):
        ...
        loader_te = DataLoader(self.handler(X, Y, transform=transform), pin_memory=True,
                            shuffle=False, **self.args.loader_te_args)
        probs = torch.zeros([len(Y), n_class])
        ...
        embeddings = torch.zeros([len(Y), embedding_dim])
        if eval:
            self.clf.eval()
            with torch.no_grad():
                for x, y, idxs in loader_te:
                    ...
                    #out, e1 = self.clf(x)
                    if(self.args.model=='LeNet'):
                        e1, _ = self.model.feature_extractor(x)
                    else:
                        e1, _ = self.model.feature_extractor(x, x.shape[2])
                    out, _ = self.model.linear(e1)
                    prob = F.softmax(out, dim=1)
                    probs[idxs] = prob.cpu()
                    # print('e1 shape = ',e1.shape)
                    # print('embed shape = ',embeddings.shape)
                    # print('embed[idxs] shape = ',embeddings[idxs].shape)
                    # sys.exit()
