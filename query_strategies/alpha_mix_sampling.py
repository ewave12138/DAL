...
class AlphaMixSampling(Strategy):
	def __init__(self, X, Y, X_te, Y_te, idxs_lb, net, handler, args):
		super(AlphaMixSampling, self).__init__( X, Y, X_te, Y_te, idxs_lb, net, handler, args)
		self.model=net
	...
	def find_alpha(self):
		assert self.args.alpha_num_mix <= self.args.n_class - (
	...
	def find_candidate_set(self, lb_embedding, ulb_embedding, pred_1, ulb_probs, alpha_cap, Y, grads):
		...
		for i in range(self.args.n_class):
			...
			if self.args.alpha_closed_form_approx:
				...
				#out, _ = self.model.linear(embedding_mix, embedding=True)
				out, _ = self.model.linear(embedding_mix.to(self.device))
				out = out.detach().cpu()
				alpha = alpha.cpu()
				...
				if self.args.alpha_opt:
					...
					#out, _ = self.model.linear(embedding_mix.to(self.device), embedding=True)
					out, _ = self.model.linear(embedding_mix.to(self.device))
					out = out.detach().cpu()
