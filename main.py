...
args_pool = {'mnist':
                { 
                 ...
                 'loader_te_args':{'batch_size': 256, 'num_workers': 8},
            ...
            'tinyimagenet': 
               {
                ...
                'transform_tr': transforms.Compose([
                 ...
                 'transform_te': transforms.Compose([
                                    transforms.ToTensor(), 
                                    transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))]),
                 'loader_tr_args':{'batch_size': 512, 'num_workers': 4},
...
def main():
    ...
    print_log("cudnn  version : {}\n".format(torch.backends.cudnn.version()), log)
    ...
    elif args.strategy == 'mobyv2al':
        strategy = query_strategies.mobyv2al(X_tr, Y_tr, X_te, Y_te, idxs_lb, net, handler, args)
    else:
    ...
    if args.load_ckpt:
        ...
        if(args.strategy == 'mobyv2al'):
            test_acc= strategy.train(alpha=alpha, n_epoch=args.n_epoch)
        else:
            strategy.train(alpha=alpha, n_epoch=args.n_epoch)
        print('first train finished') 
    if(args.strategy not in['mobyv2al'] ):
