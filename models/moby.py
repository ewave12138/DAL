...
class MoBY(nn.Module):
    ...
    def _dequeue_and_enqueue(self, keys1, keys2):
        ...
        #print(f"Batch size: {batch_size}, contrast_num_negative: {self.contrast_num_negative}")
    ...
    def forward(self, im_1, im_2, targets=None):
        ...
        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder()  # update the key encoder of constractive learning
...
def build_MoBymodel(n_class, batch_size, no_of_labelled, epochs, model_encoder):
    config = CN()
    ...
    if model_encoder == 'vgg':
        enc = resnet.dnn_16enc
        clsf = resnet.VGGNet2C(512, n_class)

    if model_encoder == 'ResNet18':
        enc = resnet.ResNet18E
        clsf = resnet.ResNetC(512, num_classes=n_class)
    if model_encoder == 'LeNet':
        enc = LeNet5
        clsf = resnet.VGGNet2C(256, num_classes=n_class)    
